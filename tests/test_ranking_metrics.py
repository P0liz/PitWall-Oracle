...
from src.ranker_features import ALL_RANKER_FEATURES, PRODUCTION_FEATURES
...
class PromotionTests(unittest.TestCase):
    def test_candidate_remains_pending_until_three_paired_races(self):
        duels = pd.DataFrame(
            {
                "champion_pairwise_accuracy": [0.7, 0.7],
                "challenger_pairwise_accuracy": [0.8, 0.8],
                "champion_teammate_pairwise_accuracy": [0.6, 0.6],
                "challenger_teammate_pairwise_accuracy": [0.7, 0.7],
                "champion_position_mae": [2.0, 2.0],
                "challenger_position_mae": [1.8, 1.8],
            }
        )
        ...
        self.assertIn("almeno 3", decision.reason)

    def test_candidate_must_not_regress_any_primary_ranker_metric(self):
        improving = pd.DataFrame(
            {
                "champion_pairwise_accuracy": [0.7, 0.8, 0.75],
                "challenger_pairwise_accuracy": [0.8, 0.8, 0.8],
                "champion_teammate_pairwise_accuracy": [0.6, 0.7, 0.6],
                "challenger_teammate_pairwise_accuracy": [0.7, 0.7, 0.7],
                "champion_position_mae": [2.0, 1.8, 2.0],
                "challenger_position_mae": [1.9, 1.8, 1.9],
            }
        )
        teammate_regression = improving.copy()
        teammate_regression["challenger_teammate_pairwise_accuracy"] = [0.5, 0.6, 0.5]
...
class ProductionFeatureTests(unittest.TestCase):
    def test_production_features_are_explicit_registered_and_unique(self):
        self.assertEqual(len(PRODUCTION_FEATURES), 20)
        self.assertEqual(len(PRODUCTION_FEATURES), len(set(PRODUCTION_FEATURES)))
        self.assertTrue(set(PRODUCTION_FEATURES).issubset(ALL_RANKER_FEATURES))
