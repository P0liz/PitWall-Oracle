...
def fit_and_log_challenger(
    history: pd.DataFrame, config: DNFModelConfig, cutoff_date: pd.Timestamp, step: int, model_dir: Path | None = None
) -> DNFFittedModel:
...
def train_and_log_dnf_model(
    dnf_df: pd.DataFrame, year: int, model_race: int, n_trials: int = DNF_OPTUNA_TRIALS, model_dir: Path | None = None
    ...
    model_dir = Path(model_dir) if model_dir is not None else MODEL_DIR
    artifact_path = model_dir / f"dnf_logistic_{year}_base.joblib"
...
def run_pipeline(model_dir: Path = MODEL_DIR):
    ...
    with mlflow.start_run(run_name=f"DNF_Season_Simulation_{NEW_YEAR}"):
        ...
        base_result, _ = train_and_log_dnf_model(dnf_history_df, NEW_YEAR, model_race=1, model_dir=model_dir)
        ...
        champion_path = model_dir / f"dnf_logistic_{NEW_YEAR}_base.joblib"
        ...
        for idx, _ in enumerate(completed_races):
            ...
            if evaluation.challenger is not None and evaluation.decision is not None:
                ...
                if evaluation.decision.promote:
                    champion_path = update_champion_after_duel(
                        champion_path, challenger_path, evaluation.decision, NEW_YEAR, race_number, model_dir=model_dir
                    )
            ...
            fit_and_log_challenger(
                dnf_history_df, frozen_config, challenger_cutoff, step=race_number, model_dir=model_dir
            )
        ...
        (model_dir / "dnf_logistic_pending.joblib").unlink(missing_ok=True)
