...
def ranker_artifact_candidates(
    artifact_paths: Iterable[str | Path], year: int, race_number: int, *, allow_legacy_base: bool = False
) -> list[str]:
    """Return local Ranker artifacts in causal preference order."""
    if race_number < 1:
    ...
    for artifact_path in artifact_paths:
        ...
        if path.name == f"pitwall_oracle_{year}_base.json":
...
def select_ranker_artifact_path(
    artifact_paths: Iterable[str | Path], year: int, race_number: int, *, allow_legacy_base: bool = False
) -> str | None:
    candidates = ranker_artifact_candidates(artifact_paths, year, race_number, allow_legacy_base=allow_legacy_base)
    return str(candidates[0]) if candidates else None


def resolve_ranker_model_path(year: int, race_number: int, local_model_dir: Path = MODEL_DIR) -> Path:
    """Resolve the newest local Ranker promoted strictly before the race."""
    local_model_dir = Path(local_model_dir)
    artifacts = list(local_model_dir.iterdir()) if local_model_dir.is_dir() else []
    selected = select_ranker_artifact_path(artifacts, year, race_number, allow_legacy_base=year == NEW_YEAR)
    if selected is None:
        raise FileNotFoundError(
            f"No causally eligible season-specific Ranker model found for year={year}, "
