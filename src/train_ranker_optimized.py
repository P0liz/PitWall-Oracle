...
from src.ranker.ranker_features import PRODUCTION_FEATURES
from src.ranker.ranking_metrics import decide_promotion, evaluate_grouped_rankings, race_ranking_metrics
from src.model_lifecycle import CALIBRATION_NAME, read_calibration, write_calibration
from src.ml_flow_auto import launch_mlflow_server, MLFLOW_HOST, MLFLOW_PORT
import asyncio
import os
import fastf1
import pandas as pd
...
MODEL_DIR = Path(os.environ.get("PITWALL_MODEL_DIR", "models"))
BASE_MODEL = f"pitwall_oracle_{NEW_YEAR}_base.json"
...
def run_pipeline(model_dir: Path = MODEL_DIR):
    ...
    with mlflow.start_run(run_name=f"F1_Season_Simulation_{NEW_YEAR}") as parent_run:
        ...
        if calibration_path.exists():
            existing_calibration = read_calibration(calibration_path).get(NEW_YEAR, {})
            base_sigma = existing_calibration.get(0, FALLBACK_SIGMA)
        else:
            base_sigma = FALLBACK_SIGMA
        calibration_values = {0: base_sigma}
        for idx, date in enumerate(races):
            ...
            calibration_values[race_idx] = sigma_statistics["cumulative_sigma_relative"]
        ...
        write_calibration(calibration_path, NEW_YEAR, calibration_values)
        (model_dir / PENDING_MODEL).unlink(missing_ok=True)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if mlflow_process:
            ...
            print("=" * 60)
            if os.environ.get("PITWALL_NON_INTERACTIVE") != "1":
                try:
