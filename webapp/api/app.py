...
from .repository import InvalidPublishedData, ResultNotFound, ResultsRepository
from .schemas import HeadToHeadResponse, HistoryDocument, HistoryIndex, PredictionDocument, SessionType

repository = ResultsRepository(Path(__file__).parent / "data")

app = FastAPI(title="PitWall Oracle API", version="1.0.0", description="Read-only Formula 1 prediction results.")
...
app.add_middleware(
    CORSMiddleware, allow_origins=allowed_origins, allow_credentials=False, allow_methods=["GET"], allow_headers=["*"]
)
...
def translate_repository_error(error: Exception) -> NoReturn:
    ...
    if isinstance(error, InvalidPublishedData):
        raise HTTPException(
            status_code=422, detail={"code": "invalid_published_data", "message": str(error)}
        ) from error
    raise error
...
def get_current_prediction(results_repository: ResultsRepository = Depends(get_repository)) -> PredictionDocument:
    try:
...
@app.get("/api/v1/predictions/current/head-to-head", response_model=HeadToHeadResponse)
def get_head_to_head(
    driver_a: str, driver_b: str, results_repository: ResultsRepository = Depends(get_repository)
) -> HeadToHeadResponse:
    if driver_a == driver_b:
        raise HTTPException(
            status_code=400,
            detail={"code": "duplicate_drivers", "message": "A driver cannot be compared with themselves"},
        )
    try:
...
def get_history(season: int, results_repository: ResultsRepository = Depends(get_repository)) -> HistoryIndex:
    try:
...
def get_history_race(
    ...
    try:
        return results_repository.load_history(season, round_number, session_type)
    except (ResultNotFound, InvalidPublishedData) as error:
        translate_repository_error(error)
